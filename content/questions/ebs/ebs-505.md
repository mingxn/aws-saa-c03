---
title: "A social media company is building a feature for its website..."
draft: false
categories:
  - EBS
  - Storage
domain: "Design High-Performing Architectures"
difficulty: "medium"
question: "A social media company is building a feature for its website. The feature will give users the ability to upload photos. The company expects signicant increases in demand during large events and must ensure that the website can handle the upload traffic from users. Which solution fulfills these requirements with the MOST scalability?"
options:
  A: "Upload files from the user's browser to the application servers. Transfer the files to an Amazon S3 bucket."
  B: "Provision an AWS Storage Gateway file gateway. Upload files directly from the user's browser to the file gateway."
  C: "Generate Amazon S3 presigned URLs in the application. Upload files directly from the user's browser into an S3 bucket."
  D: "Provision an Amazon Elastic File System (Amazon EFS) file system. Upload files directly from the user's browser to the file system."
answer: ""
explanation: ""
tags:
  - ebs
  - storage
---

<!-- Question data is in frontmatter -->
